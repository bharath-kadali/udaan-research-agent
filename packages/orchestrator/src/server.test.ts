...
const inProcess = { pipelineMode: "inprocess" as const };

describe("orchestrator API", () => {
  it("reports health", async () => {
    const app = buildServer(inProcess);
    const res = await app.inject({ method: "GET", url: "/health" });
    expect(res.statusCode).toBe(200);
    expect(res.json()).toEqual({ status: "ok", pipelineMode: "inprocess" });
    await app.close();
  });
  ...
  it("rejects an upload missing internalId", async () => {
    const app = buildServer({ ...inProcess, store: new InMemoryObjectStore() });
  ...
  it("stores a paywalled PDF upload in the vault", async () => {
    ...
    const app = buildServer({ ...inProcess, store });
