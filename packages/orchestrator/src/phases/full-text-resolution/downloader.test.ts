...
const respond = (body: Uint8Array | string, contentType: string): FetchLike => async () =>
  new Response(body, { headers: { "content-type": contentType } });

describe("downloadAndStore", () => {
  it("stores a valid PDF and reports RESOLVED_DOWNLOAD", async () => {
    const store = new InMemoryObjectStore();
    const outcome = await downloadAndStore("https://x/p.pdf", "raw_pdfs/x.pdf", store, respond(pdfBytes, "application/pdf"));
  ...
  it("flags an HTML login page as PAYWALLED", async () => {
    const store = new InMemoryObjectStore();
    const outcome = await downloadAndStore("https://x/p", "k", store, respond("<html>login</html>", "text/html"));
  ...
  it("flags a non-PDF payload as FAILED_CORRUPTED", async () => {
    const store = new InMemoryObjectStore();
    const outcome = await downloadAndStore("https://x/p.pdf", "k", store, respond(new Uint8Array([1, 2, 3]), "application/pdf"));
